...
impl OxlintConfig {
  ...
  pub fn from_options(
    output_dir: &str,
    oxlint_config_file_path: Option<&str>,
    restricted_imports: &[Restricted],
    restricted_globals: &[Restricted],
    globals: &HashMap<String, bool>,
    environments: &Environment,
  ) -> Result<Self, String> {
    if let Some(file_path) = oxlint_config_file_path {
      Self::from_file(file_path)
    } else {
      Self::generate_from_options(
        output_dir,
        restricted_imports,
        restricted_globals,
        globals,
  ...
  pub fn from_file(file_path: &str) -> Result<Self, String> {
    let oxlintrc = Oxlintrc::from_file(Path::new(file_path))
      .map_err(|e| format!("Failed to load oxlintrc file: {}, error: {:?}", file_path, e))?;

    Ok(Self {
      oxlintrc,
      source: ConfigSource::File(file_path.to_string()),
    })
  }

  /// 从选项生成配置
  fn generate_from_options(
    output_dir: &str,
    restricted_imports: &[Restricted],
    restricted_globals: &[Restricted],
    globals: &HashMap<String, bool>,
    ...
    // 2. 写入配置文件（用于调试和审计）
    let config_output_path = Path::new(output_dir).join(".oxlintrc.json");
    write_config_file(&config_json, &config_output_path, output_dir)?;

    // 3. 反序列化为 Oxlintrc
    let oxlintrc = serde_json::from_value::<Oxlintrc>(config_json)
      .map_err(|e| format!("Failed to deserialize Oxlintrc: {}", e))?;

    Ok(Self {
      oxlintrc,
      source: ConfigSource::Generated {
        output_path: config_output_path.to_string_lossy().into_owned(),
      },
    })
  }

  /// 获取 Oxlintrc 引用
  pub fn oxlintrc(&self) -> &Oxlintrc {
    &self.oxlintrc
  }
...
fn build_config_json(
  ...
  let restricted_imports = serde_json::to_value(restricted_imports)?;
  let restricted_globals = serde_json::to_value(restricted_globals)?;
  let globals = serde_json::to_value(globals)?;
...
mod tests {
  ...
  fn test_build_config_json() {
    let restricted_imports = vec![];
    let restricted_globals = vec![];
    let globals = HashMap::new();
    let environments = Environment::default();

    let result = build_config_json(&restricted_imports, &restricted_globals, &globals, &environments);
  ...
  #[test]
  fn test_config_from_options_without_file() {
    use std::env;

    let temp_dir = env::temp_dir().join("test_oxlint_config");
    let output_dir = temp_dir.to_string_lossy().to_string();

    let result = OxlintConfig::from_options(
      &output_dir,
      None,
      &[],
      &[],
    ...
    // 验证配置来源
    match config.source() {
      ConfigSource::Generated { output_path } => {
        assert!(output_path.contains(".oxlintrc.json"));
      }
      _ => panic!("Expected Generated config source"),
    }
