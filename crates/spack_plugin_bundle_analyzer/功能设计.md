# Bundle Analyzer Plugin - 全新功能设计

> **文档修订说明**: 本文档已根据实际代码 (crates/spack_plugin_bundle_analyzer/src/lib.rs) 进行核对和修正,主要修正点包括:
> - Asset.size 类型: `usize` (非 `u64`)
> - get_module_size 实现: 使用 `module.size(None, None)` (非 `original_source()`)
> - collect_assets: 添加了 source 存在性检查
> - collect_chunks: `can_be_initial` 参数使用 `&compilation.chunk_group_by_ukey`
> - get_asset_chunks: 添加了 chunk.id() 为 None 的处理
> - 函数签名细节: 使用 `&Identifier` 而非 `&ModuleIdentifier`
> - pnpm 支持: 完整支持 pnpm 路径格式并自动提取版本号
> - peer dependencies: 正确处理 pnpm peer dependencies 路径,按主版本号聚合

---

## 核心概念说明

在深入设计之前,先理解 Bundle Analyzer 插件的三个核心数据维度:

### 📦 Assets (资产/输出文件)

**是什么**: 打包后最终输出到磁盘的文件,也就是用户实际部署到服务器上的文件。

**举例**:
- `main.js` - 主入口文件 (500 KB)
- `vendor.js` - 第三方库文件 (1.2 MB)
- `styles.css` - 样式文件 (50 KB)
- `image.png` - 图片资源 (100 KB)

**为什么重要**:
- 直接影响用户下载速度和页面加载时间
- 是性能优化的最终衡量指标
- 需要控制单个文件大小 (避免超过浏览器缓存限制或 CDN 限制)

---

### 📄 Modules (模块/源文件)

**是什么**: 构建过程中处理的源代码文件,包括你写的代码和 node_modules 中的第三方库。

**举例**:
- `./src/index.js` - 你的入口文件
- `./src/components/Button.jsx` - React 组件
- `node_modules/react/index.js` - React 库的入口
- `node_modules/lodash/debounce.js` - Lodash 的防抖函数

**为什么重要**:
- 可以分析哪些源文件占用空间最大
- 可以识别不必要的依赖 (比如引入了整个 lodash 但只用了一个函数)
- 可以按包名聚合,统计第三方库的实际占用 (如 react 占 150 KB)

---

### 🧩 Chunks (代码块)

**是什么**: 打包器将 modules 组合而成的中间产物,一个 chunk 通常会生成一个 asset。

**举例**:
- `main` chunk - 包含入口模块和它的依赖,最终生成 `main.js`
- `vendor` chunk - 包含所有 node_modules,最终生成 `vendor.js`
- `lazy-page` chunk - 按需加载的页面,最终生成 `lazy-page.js`

**为什么重要**:
- 体现代码分割 (code splitting) 的效果
- 可以分析 chunk 的划分是否合理
- 可以检测重复模块 (同一个 module 出现在多个 chunks 中)

---

### 🔗 三者的关系

```
源代码层 (开发时)        打包层 (构建中)       输出层 (部署时)
┌─────────────┐         ┌─────────────┐      ┌─────────────┐
│  Module 1   │────┐    │             │      │             │
│ (index.js)  │    │    │   Chunk 1   │─────▶│  Asset 1    │
└─────────────┘    ├───▶│   (main)    │      │ (main.js)   │
┌─────────────┐    │    │             │      │             │
│  Module 2   │────┘    └─────────────┘      └─────────────┘
│ (Button.jsx)│
└─────────────┘         ┌─────────────┐      ┌─────────────┐
┌─────────────┐         │             │      │             │
│  Module 3   │────────▶│   Chunk 2   │─────▶│  Asset 2    │
│ (react)     │         │  (vendor)   │      │(vendor.js)  │
└─────────────┘         │             │      │             │
                        └─────────────┘      └─────────────┘
```

**数据流**:
1. **Module** (源文件) → 被打包器读取和解析
2. **Chunk** (代码块) → 根据入口和分割策略组合 modules
3. **Asset** (输出文件) → 从 chunk 生成最终文件并写入磁盘

**关键关系**:
- **一个 Module 可以属于多个 Chunks** (重复打包问题的根源)
- **一个 Chunk 包含多个 Modules** (聚合关系)
- **一个 Chunk 通常生成一个 Asset** (1:1 映射)
- **一个 Asset 可能关联多个 Chunks** (如 source map 文件)

---

### 🎯 用户为什么需要关注这些数据?

#### 1️⃣ **优化打包体积** - 通过 Modules 分析
- **问题**: 打包后体积过大,影响加载速度
- **分析路径**:
  - 查看哪些 modules 占用最大
  - 按包名聚合,找出最重的第三方库
  - 评估是否可以替换或按需引入

**实际案例**:
```
发现 moment.js 占 500 KB → 替换为 day.js 只占 2 KB
发现引入了整个 lodash → 改为按需引入 lodash-es
```

#### 2️⃣ **检测重复打包** - 通过 Chunks 和 Modules 关系
- **问题**: 同一个 module 被打包到多个 chunks,浪费空间
- **分析路径**:
  - 统计哪些 modules 出现在多个 chunks 中
  - 计算浪费的大小 = module.size × (出现次数 - 1)
  - 调整 splitChunks 配置,抽取公共模块

**实际案例**:
```
React 同时出现在 main.js 和 page1.js 中
→ 配置 splitChunks 抽取到 vendor.js
→ 节省 150 KB 重复体积
```

#### 3️⃣ **验证代码分割效果** - 通过 Chunks 和 Assets
- **问题**: 不确定懒加载/按需加载是否生效
- **分析路径**:
  - 查看是否生成了预期的 lazy chunks
  - 检查 chunk 大小是否合理 (不宜过大或过小)
  - 验证 initial chunks 数量是否符合预期

**实际案例**:
```
配置了路由懒加载,但只看到一个 main.js
→ 检查发现配置错误,修改后生成了多个 lazy-*.js
→ 首屏加载从 2MB 降到 500KB
```

#### 4️⃣ **制定性能预算** - 通过 Assets
- **问题**: 需要控制资源大小,避免超标
- **分析路径**:
  - 统计每个 asset 的大小
  - 对比性能预算阈值 (如主 bundle < 200KB)
  - 在 CI/CD 中设置性能门禁

**实际案例**:
```
设置规则: main.js 不能超过 300KB
→ 每次构建自动检查
→ 超标时构建失败并提示优化
```

#### 5️⃣ **可视化依赖关系** - 综合三者
- **问题**: 不清楚代码之间的依赖关系和打包结构
- **分析路径**:
  - 生成树状图 (treemap),按层级展示 modules
  - 按颜色区分文件类型 (JS/CSS/图片)
  - 可交互查看每个 module 属于哪些 chunks

**实际价值**:
```
直观看到 node_modules 占了 80% 体积
点击某个大文件,查看是哪个 module 引入的
追溯依赖链,找到优化切入点
```

---

## 设计哲学

假设你一行代码都没写，我们从零开始思考：**一个 Bundle Analyzer 插件的核心价值是什么？**

答案是：**帮助开发者回答这些问题**

- 我的打包产物有多大？
- 哪些文件/模块占用了最多空间？
- 我引入的第三方库（node_modules）各自占用多大？
- 代码分割（code splitting）是否生效？
- 有没有重复打包的模块？

基于这个思路，我认为这个插件应该是一个**数据收集器 + 分析器**，它的输出应该能够：

1. 直接阅读（JSON 格式）
2. 可视化展示（提供给前端）
3. 集成到 CI/CD（性能门禁）

---

## 核心功能设计

### 阶段一：最小可用版本（MVP）

**目标**：实现一个能用的版本，输出可读的分析报告

#### 功能 1.1：基础数据收集

**要收集什么？**

```rust
// 核心数据结构
struct BundleReport {
  // 元数据
  timestamp: u64,
  rspack_version: String,

  // 总览
  summary: Summary,

  // 资产（最终输出的文件）
  assets: Vec<Asset>,

  // 模块（源代码文件）
  modules: Vec<Module>,

  // 代码块
  chunks: Vec<Chunk>,
}

struct Summary {
  total_size: u64,        // 总大小（字节）
  total_assets: usize,    // 输出文件数量
  total_modules: usize,   // 模块数量
  total_chunks: usize,    // chunk 数量
  build_time: f64,        // 构建耗时（毫秒）
}

struct Asset {
  name: String,           // 文件名，如 "main.js"
  size: usize,            // 文件大小(注意:实际代码中是 usize,不是 u64)
  chunks: Vec<String>,    // 关联的 chunk
  emitted: bool,          // 是否实际输出
}

struct Module {
  id: String,             // 模块唯一 ID
  name: String,           // 可读名称，如 "./src/index.js"
  size: u64,              // 模块大小
  chunks: Vec<String>,    // 包含此模块的 chunks
}

struct Chunk {
  id: String,             // chunk ID
  names: Vec<String>,     // chunk 名称
  size: u64,              // chunk 大小
  modules: Vec<String>,   // 包含的模块 ID 列表
  entry: bool,            // 是否入口 chunk
  initial: bool,          // 是否初始 chunk
}

struct Package {
  name: String,           // 包名,如 "react" 或 "@babel/core"
  version: String,        // 版本号(pnpm 可从路径提取,npm/yarn 为 "unknown")
  size: u64,              // 该包的总大小
  module_count: usize,    // 包含的模块数量
  modules: Vec<String>,   // 该包包含的所有模块 ID 列表
}
```

**实现要点**：

```rust
// 在 after_emit hook 中收集数据
async fn after_emit(&self, compilation: &mut Compilation) -> Result<()> {
  let start = Instant::now();

  // 1. 收集 Assets（输出文件）
  let assets = collect_assets(compilation);

  // 2. 收集 Modules（源文件）
  let modules = collect_modules(compilation);

  // 3. 收集 Chunks（代码块）
  let chunks = collect_chunks(compilation);

  // 4. 分析 Packages（按包名聚合）
  let packages = analyze_packages(&modules);

  // 5. 生成摘要
  let summary = Summary {
    total_size: assets.iter().map(|a| a.size).sum(),
    total_assets: assets.len(),
    total_modules: modules.len(),
    total_chunks: chunks.len(),
    build_time: start.elapsed().as_millis() as f64,
  };

  // 6. 构建报告
  let report = BundleReport {
    timestamp: current_timestamp(),
    rspack_version: get_rspack_version(),
    summary,
    assets,
    modules,
    chunks,
    packages,
  };

  // 7. 输出报告
  output_report(&report, &self.options);

  Ok(())
}
```

**关键实现**：

```rust
fn collect_assets(compilation: &Compilation) -> Vec<Asset> {
  compilation
    .assets()
    .iter()
    .map(|(name, asset)| {
      // 注意:需要检查 source 是否存在,避免 panic
      let size = if let Some(source) = &asset.source {
        source.size()
      } else {
        0
      };

      Asset {
        name: name.to_string(),
        size: size,  // 直接使用 usize,不需要 as u64
        chunks: get_asset_chunks(name, compilation),
        emitted: true,
      }
    })
    .collect()
}

fn collect_modules(compilation: &Compilation) -> Vec<Module> {
  let module_graph = compilation.get_module_graph();
  let chunk_graph = &compilation.chunk_graph;

  module_graph
    .modules()
    .into_iter()  // 注意:实际代码使用 into_iter(),不是 map()
    .map(|(id, module)| Module {
      id: id.to_string(),
      name: module
        .readable_identifier(&compilation.options.context)
        .to_string(),
      size: get_module_size(module.as_ref()),
      chunks: get_module_chunks(&id, chunk_graph),  // 注意:传入的是 &id
    })
    .collect()
}

fn collect_chunks(compilation: &Compilation) -> Vec<Chunk> {
  let chunk_graph = &compilation.chunk_graph;
  let module_graph = compilation.get_module_graph();

  compilation
    .chunk_by_ukey
    .iter()
    .map(|(ukey, chunk)| {
      let modules: Vec<String> = chunk_graph  // 注意:添加了类型标注
        .get_chunk_modules(ukey, &module_graph)
        .iter()
        .map(|m| m.identifier().to_string())
        .collect();

      Chunk {
        id: ukey.as_u32().to_string(),
        names: chunk
          .name()
          .map(|n| vec![n.to_string()])
          .unwrap_or_default(),
        size: calculate_chunk_size(&modules, &module_graph),
        modules,
        entry: chunk.has_entry_module(chunk_graph),
        // 注意:实际代码传入的是 &compilation.chunk_group_by_ukey,不是 chunk_graph
        initial: chunk.can_be_initial(&compilation.chunk_group_by_ukey),
      }
    })
    .collect()
}

// 获取模块真实大小
fn get_module_size(module: &dyn Module) -> u64 {
  // 注意:实际代码使用 Module trait 的 size 方法,不是 original_source()
  // source_type 参数为 None 表示获取所有类型的总大小
  // compilation 参数为 None 因为我们不需要编译上下文
  module.size(None, None) as u64
}

// 分析包依赖,按包名聚合
fn analyze_packages(modules: &[Module]) -> Vec<Package> {
  use std::collections::HashMap;

  // key 是包名, value 是 (版本号, 模块列表)
  let mut package_map: HashMap<String, (String, Vec<&Module>)> = HashMap::new();

  // 1. 遍历所有模块,按包名分组
  for module in modules {
    if let Some((package_name, version)) = parse_package_info(&module.name) {
      package_map
        .entry(package_name)
        .or_insert_with(|| (version.clone(), Vec::new()))
        .1
        .push(module);
    }
  }

  // 2. 为每个包生成统计信息
  let mut packages: Vec<Package> = package_map
    .into_iter()
    .map(|(name, (version, mods))| {
      let size: u64 = mods.iter().map(|m| m.size).sum();
      let modules: Vec<String> = mods.iter().map(|m| m.id.clone()).collect();

      Package {
        name,
        version,  // 从路径中提取的版本号
        size,
        module_count: mods.len(),
        modules,
      }
    })
    .collect();

  // 3. 按大小降序排序
  packages.sort_by_key(|p| std::cmp::Reverse(p.size));

  packages
}

// 关于 pnpm peer dependencies 的聚合策略:
// 同一个包的不同 peer deps 变体会被合并为一条记录
// 例如:
//   lodash@4.17.21_react@18.0.0  ┐
//   lodash@4.17.21_react@19.0.0  ├─> 合并为: Package { name: "lodash", version: "4.17.21", ... }
//   lodash@4.17.21              ┘
// 理由:
// 1. 用户视角: 只关心"lodash 4.17.21 总共占用多大空间"
// 2. 一致性: npm/yarn/pnpm 使用统一的数据结构
// 3. 简化统计: 避免同一个包出现多次导致混乱

// 从模块路径中解析包名和版本号
// 支持 npm/yarn 和 pnpm 两种路径格式
// 例如:
//   npm/yarn: "node_modules/react/index.js" -> Some(("react", "unknown"))
//   pnpm: "node_modules/.pnpm/react@18.2.0/node_modules/react/index.js"
//         -> Some(("react", "18.2.0"))
//   pnpm scoped: "node_modules/.pnpm/@babel+core@7.22.0/node_modules/@babel/core/lib.js"
//                -> Some(("@babel/core", "7.22.0"))
//   pnpm peer deps: "node_modules/.pnpm/@visactor+react-vchart@2.0.9_react@19.2.0/..."
//                   -> Some(("@visactor/react-vchart", "2.0.9"))
//                   注意:忽略 _peer 后缀,只提取主版本号
fn parse_package_info(module_path: &str) -> Option<(String, String)> {
  if !module_path.contains("node_modules/") {
    return None;
  }

  // 优先检查 pnpm 格式
  if module_path.contains("node_modules/.pnpm/") {
    return parse_pnpm_package_info(module_path);
  }

  // 处理标准 npm/yarn 格式
  parse_npm_package_info(module_path)
}

// 解析 pnpm 格式: node_modules/.pnpm/包名@版本/node_modules/包名/...
// 处理 peer dependencies: 包名@版本_peer依赖信息
fn parse_pnpm_package_info(module_path: &str) -> Option<(String, String)> {
  let parts: Vec<&str> = module_path.split("node_modules/.pnpm/").collect();
  if parts.len() < 2 { return None; }

  let pkg_with_version = parts[1].split('/').next()?;

  // 先去掉 peer dependencies 后缀 (如果有的话)
  // 例如: react@18.2.0_some_peer -> react@18.2.0
  let pkg_without_peers = if let Some(underscore_pos) = pkg_with_version.find('_') {
    &pkg_with_version[..underscore_pos]
  } else {
    pkg_with_version
  };

  // scoped package: @babel+core@7.22.0
  if pkg_without_peers.starts_with('@') {
    let without_at = &pkg_without_peers[1..];
    let last_at_pos = without_at.rfind('@')?;
    let name_part = &without_at[..last_at_pos];
    let version = &without_at[last_at_pos + 1..];
    let package_name = format!("@{}", name_part.replace('+', "/"));
    return Some((package_name, version.to_string()));
  }

  // 普通 package: react@18.2.0
  let at_pos = pkg_without_peers.rfind('@')?;
  Some((
    pkg_without_peers[..at_pos].to_string(),
    pkg_without_peers[at_pos + 1..].to_string(),
  ))
}

// 解析 npm/yarn 格式: node_modules/包名/...
fn parse_npm_package_info(module_path: &str) -> Option<(String, String)> {
  let parts: Vec<&str> = module_path.split("node_modules/").collect();
  if parts.len() < 2 { return None; }

  let after_nm = parts[parts.len() - 1];
  let segments: Vec<&str> = after_nm.split('/').collect();

  if segments[0].starts_with('@') {
    if segments.len() < 2 { return None; }
    Some((format!("{}/{}", segments[0], segments[1]), "unknown".to_string()))
  } else {
    Some((segments[0].to_string(), "unknown".to_string()))
  }
}
```

**输出格式**：

```rust
fn output_report(report: &BundleReport, options: &PluginOptions) {
  // 1. 输出 JSON 文件
  if let Some(path) = &options.report_filename {
    let json = serde_json::to_string_pretty(report).unwrap();
    std::fs::write(path, json).unwrap();
    println!("📊 Bundle analysis saved to: {}", path);
  }

  // 2. 调用回调
  if let Some(callback) = &options.on_analyzed {
    callback(report.clone()).await;
  }
}
```

---

#### 功能 1.2：简单的终端输出

**目标**：构建完成后直接在终端显示关键信息

```rust
fn print_summary(report: &BundleReport) {
  use byte_unit::{Byte, UnitType};

  println!("\n📦 Bundle Analysis Summary");
  println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");

  let total = Byte::from_u64(report.summary.total_size);
  println!("Total Size:    {}", total.get_appropriate_unit(UnitType::Binary));
  println!("Assets:        {}", report.summary.total_assets);
  println!("Modules:       {}", report.summary.total_modules);
  println!("Chunks:        {}", report.summary.total_chunks);
  println!("Build Time:    {:.2}ms", report.summary.build_time);

  println!("\n📄 Largest Assets:");
  let mut assets = report.assets.clone();
  assets.sort_by_key(|a| std::cmp::Reverse(a.size));

  for asset in assets.iter().take(5) {
    let size = Byte::from_u64(asset.size);
    println!("  {} - {}", asset.name, size.get_appropriate_unit(UnitType::Binary));
  }

  println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
}
```

---

### 阶段二：深度分析

**目标**：提供更有价值的分析数据

#### 功能 2.1：依赖包分析（最实用！）

**为什么重要？**

- node_modules 通常占用 70%+ 的体积
- 开发者需要知道哪个包最重，考虑是否替换

**数据结构**：

```rust
struct PackageAnalysis {
  packages: Vec<PackageInfo>,
  total_size: u64,
  total_count: usize,
}

struct PackageInfo {
  name: String,              // 包名，如 "react"
  version: String,           // 版本，如 "18.2.0"
  size: u64,                 // 占用大小
  module_count: usize,       // 包含的模块数
  modules: Vec<String>,      // 模块路径列表
  main_fields: Vec<String>,  // 实际使用的入口
}
```

**实现思路**：

```rust
fn analyze_packages(modules: &[Module]) -> PackageAnalysis {
  let mut package_map: HashMap<String, Vec<&Module>> = HashMap::new();

  // 1. 分组：按包名聚合模块
  for module in modules {
    if let Some((package_name, _)) = parse_package_path(&module.name) {
      package_map.entry(package_name).or_default().push(module);
    }
  }

  // 2. 统计每个包的信息
  let mut packages: Vec<PackageInfo> = package_map
    .into_iter()
    .map(|(name, mods)| {
      let version = resolve_package_version(&name);  // 从 package.json 读取
      let size = mods.iter().map(|m| m.size).sum();

      PackageInfo {
        name: name.clone(),
        version,
        size,
        module_count: mods.len(),
        modules: mods.iter().map(|m| m.name.clone()).collect(),
        main_fields: vec![], // 可选
      }
    })
    .collect();

  // 3. 按大小排序
  packages.sort_by_key(|p| std::cmp::Reverse(p.size));

  PackageAnalysis {
    total_size: packages.iter().map(|p| p.size).sum(),
    total_count: packages.len(),
    packages,
  }
}

// 解析包路径
// "node_modules/react/index.js" -> ("react", "index.js")
// "node_modules/@babel/core/lib/index.js" -> ("@babel/core", "lib/index.js")
fn parse_package_path(path: &str) -> Option<(String, String)> {
  let parts: Vec<&str> = path.split("node_modules/").collect();
  if parts.len() < 2 {
    return None;
  }

  let after_nm = parts[1];
  let segments: Vec<&str> = after_nm.split('/').collect();

  let (pkg_name, rest) = if segments[0].starts_with('@') {
    // scoped package: @babel/core
    let name = format!("{}/{}", segments[0], segments.get(1)?);
    let rest = segments[2..].join("/");
    (name, rest)
  } else {
    // normal package: react
    let name = segments[0].to_string();
    let rest = segments[1..].join("/");
    (name, rest)
  };

  Some((pkg_name, rest))
}

// 读取 package.json 获取版本
fn resolve_package_version(package_name: &str) -> String {
  // 1. 从项目根目录开始查找
  // 2. 读取 node_modules/{package_name}/package.json
  // 3. 解析 version 字段

  // 简化实现
  "unknown".to_string()
}
```

**终端输出**：

```rust
fn print_package_analysis(analysis: &PackageAnalysis) {
  use byte_unit::{Byte, UnitType};

  println!("\n📦 Top 10 Packages by Size:");
  println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");

  for (i, pkg) in analysis.packages.iter().take(10).enumerate() {
    let size = Byte::from_u64(pkg.size);
    let percentage = (pkg.size as f64 / analysis.total_size as f64) * 100.0;

    println!(
      "{:2}. {:30} {:>10} ({:>5.1}%) - {} modules",
      i + 1,
      format!("{}@{}", pkg.name, pkg.version),
      size.get_appropriate_unit(UnitType::Binary).to_string(),
      percentage,
      pkg.module_count
    );
  }

  println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
}
```

---

#### 功能 2.2：重复模块检测

**为什么重要？**

- 同一个模块被多个 chunk 包含 = 浪费空间
- 应该抽取到公共 chunk

**数据结构**：

```rust
struct DuplicateAnalysis {
  duplicates: Vec<DuplicateModule>,
  wasted_size: u64,  // 浪费的总大小
}

struct DuplicateModule {
  module_id: String,
  module_name: String,
  size: u64,
  occurrences: usize,     // 出现次数
  chunks: Vec<String>,    // 包含它的 chunks
  wasted_size: u64,       // 浪费的大小 = size * (occurrences - 1)
}
```

**实现**：

```rust
fn detect_duplicates(modules: &[Module], chunks: &[Chunk]) -> DuplicateAnalysis {
  // 1. 统计每个模块出现在多少个 chunk 中
  let mut module_chunks: HashMap<String, Vec<String>> = HashMap::new();

  for chunk in chunks {
    for module_id in &chunk.modules {
      module_chunks
        .entry(module_id.clone())
        .or_default()
        .push(chunk.id.clone());
    }
  }

  // 2. 找出重复的模块（出现在 2+ chunks）
  let duplicates: Vec<DuplicateModule> = module_chunks
    .into_iter()
    .filter(|(_, chunks)| chunks.len() > 1)
    .filter_map(|(module_id, chunks)| {
      let module = modules.iter().find(|m| m.id == module_id)?;
      let occurrences = chunks.len();
      let wasted = module.size * (occurrences as u64 - 1);

      Some(DuplicateModule {
        module_id: module_id.clone(),
        module_name: module.name.clone(),
        size: module.size,
        occurrences,
        chunks,
        wasted_size: wasted,
      })
    })
    .collect();

  let wasted_size = duplicates.iter().map(|d| d.wasted_size).sum();

  DuplicateAnalysis {
    duplicates,
    wasted_size,
  }
}
```

**终端输出**：

```rust
fn print_duplicate_analysis(analysis: &DuplicateAnalysis) {
  if analysis.duplicates.is_empty() {
    println!("✅ No duplicate modules found!");
    return;
  }

  use byte_unit::{Byte, UnitType};

  println!("\n⚠️  Duplicate Modules Detected:");
  println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
  println!(
    "Total wasted: {}",
    Byte::from_u64(analysis.wasted_size).get_appropriate_unit(UnitType::Binary)
  );

  let mut sorted = analysis.duplicates.clone();
  sorted.sort_by_key(|d| std::cmp::Reverse(d.wasted_size));

  for dup in sorted.iter().take(10) {
    let size = Byte::from_u64(dup.size);
    let wasted = Byte::from_u64(dup.wasted_size);

    println!(
      "\n  {} ({} -> wasted: {})",
      dup.module_name,
      size.get_appropriate_unit(UnitType::Binary),
      wasted.get_appropriate_unit(UnitType::Binary)
    );
    println!("  Appears in {} chunks: {:?}", dup.occurrences, dup.chunks);
  }

  println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
}
```

---

### 阶段三：可视化支持

**目标**：生成前端可直接使用的数据格式

#### 功能 3.1：树状图数据（Treemap）

**用途**：用于 echarts/d3.js 绘制树状图

```rust
struct TreemapData {
  name: String,
  value: u64,              // 大小
  children: Vec<TreemapData>,
  path: Option<String>,    // 完整路径
  item_style: Option<ItemStyle>,
}

struct ItemStyle {
  color: String,  // 根据类型着色
}

fn generate_treemap(modules: &[Module]) -> TreemapData {
  let mut root = TreemapNode::new("root");

  // 按路径构建树
  for module in modules {
    insert_module_into_tree(&mut root, module);
  }

  build_treemap_data(root)
}

struct TreemapNode {
  name: String,
  size: u64,
  children: HashMap<String, TreemapNode>,
  is_leaf: bool,
  module_type: Option<String>,
}

fn insert_module_into_tree(root: &mut TreemapNode, module: &Module) {
  let path = &module.name;
  let segments: Vec<&str> = path.split('/').filter(|s| !s.is_empty()).collect();

  let mut current = root;

  for (i, segment) in segments.iter().enumerate() {
    let is_last = i == segments.len() - 1;

    current.size += module.size;

    if is_last {
      // 叶子节点
      let mut leaf = TreemapNode::new(segment);
      leaf.size = module.size;
      leaf.is_leaf = true;
      leaf.module_type = Some(detect_module_type(path));
      current.children.insert(segment.to_string(), leaf);
    } else {
      // 中间节点
      current = current
        .children
        .entry(segment.to_string())
        .or_insert_with(|| TreemapNode::new(segment));
    }
  }
}

fn build_treemap_data(node: TreemapNode) -> TreemapData {
  let children: Vec<TreemapData> = node
    .children
    .into_iter()
    .map(|(_, child)| build_treemap_data(child))
    .collect();

  let color = if let Some(ref module_type) = node.module_type {
    get_color_for_type(module_type)
  } else {
    "#cccccc".to_string()
  };

  TreemapData {
    name: node.name,
    value: node.size,
    children,
    path: None,
    item_style: Some(ItemStyle { color }),
  }
}

fn get_color_for_type(module_type: &str) -> String {
  match module_type {
    "javascript" => "#f7df1e".to_string(),
    "typescript" => "#3178c6".to_string(),
    "css" => "#264de4".to_string(),
    "image" => "#4caf50".to_string(),
    "json" => "#000000".to_string(),
    _ => "#999999".to_string(),
  }
}

fn detect_module_type(path: &str) -> String {
  if path.ends_with(".ts") || path.ends_with(".tsx") {
    "typescript"
  } else if path.ends_with(".js") || path.ends_with(".jsx") {
    "javascript"
  } else if path.ends_with(".css") || path.ends_with(".scss") {
    "css"
  } else if path.ends_with(".png") || path.ends_with(".jpg") || path.ends_with(".svg") {
    "image"
  } else if path.ends_with(".json") {
    "json"
  } else {
    "other"
  }.to_string()
}
```

---

### 阶段四：配置与输出

**目标**：灵活的配置选项

```rust
pub struct BundleAnalyzerPluginOpts {
  // 输出选项
  pub report_filename: Option<String>,  // JSON 报告路径
  pub open_analyzer: bool,              // 是否自动打开可视化页面

  // 分析选项
  pub analyze_mode: AnalyzeMode,
  pub exclude_assets: Option<Vec<String>>,  // 排除的资源（glob）
  pub stats_options: StatsOptions,

  // 回调
  pub on_analyzed: Option<CompilationHookFn>,
}

pub enum AnalyzeMode {
  Full,      // 完整分析（包含所有功能）
  Static,    // 只生成静态报告
  Disabled,  // 禁用
}

pub struct StatsOptions {
  pub show_packages: bool,      // 是否分析 packages
  pub show_duplicates: bool,    // 是否检测重复
  pub top_modules_count: usize, // 显示 top N 模块
}

impl Default for BundleAnalyzerPluginOpts {
  fn default() -> Self {
    Self {
      report_filename: Some("bundle-report.json".to_string()),
      open_analyzer: false,
      analyze_mode: AnalyzeMode::Full,
      exclude_assets: None,
      stats_options: StatsOptions {
        show_packages: true,
        show_duplicates: true,
        top_modules_count: 20,
      },
      on_analyzed: None,
    }
  }
}
```

---

## 最终的数据结构

```rust
// 最终输出的完整报告
#[derive(Serialize, Deserialize)]
pub struct BundleReport {
  // 基础信息
  pub timestamp: u64,
  pub rspack_version: String,
  pub summary: Summary,

  // 核心数据
  pub assets: Vec<Asset>,
  pub modules: Vec<Module>,
  pub chunks: Vec<Chunk>,

  // 分析结果
  pub package_analysis: Option<PackageAnalysis>,
  pub duplicate_analysis: Option<DuplicateAnalysis>,

  // 可视化数据
  pub treemap_data: Option<TreemapData>,
}
```

---

## 实施计划

### 第一步：实现 MVP（阶段一）

- [ ] 定义基础数据结构（Asset、Module、Chunk、Summary）
- [ ] 实现 `collect_assets()`
- [ ] 实现 `collect_modules()`（准确的大小计算）
- [ ] 实现 `collect_chunks()`
- [ ] 实现 JSON 输出
- [ ] 实现简单的终端摘要输出

**验收标准**：

- 能输出 bundle-report.json
- 终端显示总大小、文件数等基础信息
- 数据准确（和实际文件大小一致）

---

### 第二步：添加深度分析（阶段二）

- [ ] 实现包分析（`analyze_packages()`）
- [ ] 实现 `parse_package_path()`
- [ ] 实现 `resolve_package_version()`（读取 package.json）
- [ ] 实现重复检测（`detect_duplicates()`）
- [ ] 美化终端输出

**验收标准**：

- 终端显示 Top 10 最大的包
- 显示重复模块及浪费的空间
- 数据可读性好

---

### 第三步：可视化数据（阶段三）

- [ ] 实现树状图数据生成
- [ ] 支持按类型着色
- [ ] （可选）生成 HTML 可视化页面

**验收标准**：

- JSON 中包含 treemap_data
- 数据可以直接用于 echarts

---

### 第四步：完善配置（阶段四）

- [ ] 实现配置选项
- [ ] 支持排除某些资源
- [ ] 支持不同的分析模式

**验收标准**：

- 配置灵活可用
- 文档完善

---

## 关键代码片段

### 准确获取模块大小

```rust
fn get_module_size(module: &dyn Module) -> u64 {
  // 实际实现:使用 Module trait 的 size 方法
  // source_type 参数为 None 表示获取所有类型的总大小
  // compilation 参数为 None 因为我们不需要编译上下文
  module.size(None, None) as u64
}
```

### 获取 Asset 关联的 Chunks

```rust
fn get_asset_chunks(asset_name: &str, compilation: &Compilation) -> Vec<String> {
  compilation
    .chunk_by_ukey
    .values()
    .filter(|chunk| chunk.files().contains(asset_name))  // 实际代码不需要 .into()
    .map(|chunk| {
      // 实际代码需要处理 chunk.id() 为 None 的情况
      let id = if let Some(id) = chunk.id() {
        id.to_string()
      } else {
        "".to_string()
      };
      return id;
    })
    .collect()
}
```

### 获取 Module 所在的 Chunks

```rust
fn get_module_chunks(module_id: &Identifier, chunk_graph: &ChunkGraph) -> Vec<String> {
  // 注意:实际代码中参数类型是 &Identifier,不是 &ModuleIdentifier
  chunk_graph
    .get_module_chunks(*module_id)
    .iter()
    .map(|chunk_ukey| chunk_ukey.as_u32().to_string())
    .collect()
}
```

---

## 总结

这个设计的核心思路：

1. **从简单开始**：先实现 MVP，保证数据准确
2. **逐步增强**：每个阶段都增加一个实用功能
3. **注重实用性**：包分析、重复检测都是真正有用的
4. **可视化友好**：输出的数据可以直接用于前端展示

建议按照**第一步 → 第二步 → 第三步 → 第四步**的顺序实施，每完成一步就有一个可用的版本。
