...
interface TableProps extends React.ComponentProps<'table'> {
  /**
   * 最大高度 - 启用后表头固定，表体可滚动
   * @example maxHeight="h-108" maxHeight="max-h-96"
   */
  maxHeight?: string
}

function Table({ className, maxHeight, ...props }: TableProps) {
  return (
    <div
      data-slot="table-container"
      className={cn('relative w-full overflow-x-auto', maxHeight && 'overflow-y-auto', maxHeight)}
    >
